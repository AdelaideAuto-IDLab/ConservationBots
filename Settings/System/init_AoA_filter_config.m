...
function filter_config = init_AoA_filter_config(varargin)

    %% --- Instantiate inputParser
    p = inputParser;
    addParameter(p, 'cz', 1);        % aoa (bearing) false alarm pdf
    addParameter(p, 'nx', 3);        % number of state variable
    addParameter(p, 'Pb', 1e-3);     % birth probability
    addParameter(p, 'Pd', 0.99);     % aoa (bearing) detection probability
    addParameter(p, 'Pf', 1e-2);     % aoa false alarm rate
