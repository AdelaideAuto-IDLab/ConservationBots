function [config] = Experiment_config_wrapper_Swan_Reach(ntarget, reward, Use_Void, rotation_time)
    ...
    imprecision_range = repmat([-10; 3], 1,ntarget);   % above ground
    noise_q = 1;
    vmax = 5;
    DEM_file = 'Swan_Reach_DEM';
    sensitivity = -75;
%     target_id_list = [4,6, 7,8, 9];
    target_id_list = [4,7];
    ...
    %% select search area
    area_gps = get_gps_boundary('Swan_Reach_3_Feb');   % small area comtain tagged wombats


    birth_state = repmat([50, 50, 0.8]', 1, length(target_id_list));
%     P0_list =  -30+[40, 42, 57, 42]; % 10 tag detector [4, 67,8], 03/Feb
%     P0_list =  -30+[39, 42, 58, 42, 42]; % 10 tag detector [4,7], 03/Feb
    P0_list =  -30+[39, 58]; % 2 underground tag, 03/Feb
    n_list = 3.5*ones(1, length(target_id_list));   
    ...
    path_loss_truth = 3.5*ones(1,ntarget);
