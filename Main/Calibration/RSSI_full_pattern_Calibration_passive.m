...
while (t < Total_time)
    ...
    for n = 1:ntarget 
        ...
        if ~isempty(target_idx)
            ...
            theta(n, t) = atan2(truth(1, n) - squeeze(uav(n, 1, t)), truth(2, n) - squeeze(uav(n, 2, t)) );
            d(n, t) = sqrt(sum((squeeze(uav(n, 1:3, t)') - truth(1:3, n)).^2));
            tmp_d = sqrt(sum((squeeze(uav(n, 1:2, t)') - truth(1:2, n)).^2));
            phi(n, t) = atan2(truth(3, n) - squeeze(uav(n, 3, t)), tmp_d);
        end
    end
    ...
    % ==================== Update Plot =====================
    for n = 1:ntarget
        set(hp, 'XData', 10*sin(theta(n, :)), 'YData', 10*cos(theta(n,:)), 'ZData', meas(n, :) + 10*log10(d(n, :))*Path_loss_n );       
    end
