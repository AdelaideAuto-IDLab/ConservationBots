...
Total_time = res.Total_time;                      % maximun run time
...
config  = res.config;   % use saved config
...
wanted_ID_list = res.wanted_ID_list;
truth_gps = res.truth_gps;

% ==== Uncomment for static target ==========
% Convert truth to correct format
...
RSSI_model.config.target_id_list = config.target_id_list;
%% ============== Plotting setting ==================
if plot_result
    if plot_gps
...
recorder.cleanup(t, res.planning_cycle);
