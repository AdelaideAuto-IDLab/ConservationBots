classdef Result_plot_experiment < handle
    properties     
        fig_handle;
        app;                    
        config;
    ...
    methods 
        function obj = Result_plot_experiment(max_target, varargin)
            p = inputParser;
            ...
            if ~obj.config.Plot3D
                ...
                obj.UAV_shape_handle = UAV_shape(obj.app.UIAxes_Main, 'k', 10);
                ...
                % plot boundary
                obj.boundary_handle = plot(polyshape(obj.config.area'),  'EdgeColor', 'k', 'LineWidth', 2, 'FaceAlpha', 0.2, 'Parent', obj.app.UIAxes_Main);
                ...
                if ~isempty(obj.config.area)
                    xlim(obj.app.UIAxes_Main, [min(obj.config.area(1,:)) - 50, max(obj.config.area(1,:))+50]);
                    ylim(obj.app.UIAxes_Main, [min(obj.config.area(2,:)) - 50, max(obj.config.area(2,:))+50]);
                else
                xlim(obj.app.UIAxes_Main, [-50, size(obj.config.DEM, 2)+50]);
                ylim(obj.app.UIAxes_Main, [-50, size(obj.config.DEM, 1)+50]);
                end
                
                
                obj.error_xy_handle = gobjects(obj.config.max_target, 1);
                for i = 1:obj.config.max_target
                    obj.error_xy_handle(i) = plot(nan, nan, 'LineStyle', '-', 'Color', obj.color(i, :), 'LineWidth', obj.linewidth, 'Parent', obj.app.UIAxes_RMSE); 
                end
                title(obj.app.UIAxes_RMSE, 'XY Error'); xlabel(obj.app.UIAxes_RMSE, 'times (s)'); ylabel(obj.app.UIAxes_RMSE, 'Error (m)'); grid(obj.app.UIAxes_RMSE, 'on');
